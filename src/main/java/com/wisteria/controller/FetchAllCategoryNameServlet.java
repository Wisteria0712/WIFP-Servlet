...
public class FetchAllCategoryNameServlet extends HttpServlet {
    ...
    protected void doGet(@NotNull HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("FetchAllCategoryNameServlet.doGet access");
        List<String> categoryNameList = noteService.fetchAllCategoryNameOnly();
        req.setAttribute("categoryNameList", categoryNameList);
        req.getRequestDispatcher("/IndexServlet.tran?url=/author/categoryNameList.jsp").forward(req, resp);
    }
